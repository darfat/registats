...
/**
 * REST controller for managing {@link com.darfat.registats.domain.Competition}.
 */
@RestController
@RequestMapping("/api")
public class CompetitionResource {
    ...
    public ResponseEntity<List<Competition>> searchCompetitions(@RequestParam String query, Pageable pageable) {
        ...
        Page<Competition> page = competitionSearchRepository.search(queryStringQuery(query), pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
